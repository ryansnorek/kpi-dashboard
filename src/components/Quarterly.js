import Meter from "./charts/Meter";
import { getQuarter } from "../helper";

function Quarterly() {
  const currentQuarter = getQuarter();
  const qtrID = (qtr) => `${currentQuarter === qtr && "highlight"}`;

  return (
    <section className="quarterly">
      <div className="period">
        <button id={qtrID("1")}>1Q22</button>
        <button id={qtrID("2")}>2Q22</button>
        <button id={qtrID("3")}>3Q22</button>
        <button id={qtrID("4")}>4Q22</button>
      </div>
      <div className="meters">
        <Meter title="Estimated" meterValue={69} color={"rgb(218, 218, 218)"} />
        <Meter title="Guaranteed" meterValue={80} color={"rgb(15, 54, 113)"} />
        <Meter title="Predicted" meterValue={106} color={"rgb(241, 210, 34)"} />
      </div>
    </section>
  );
}
export default Quarterly;
